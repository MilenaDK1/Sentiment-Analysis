...
def read_and_train(VectorSize,WindowSize):
...
def read_csv():
	...
	with open('imdb_master.csv/imdb_master.csv', encoding="ISO-8859-1") as csvfile:
...
# ALTERAR OS NOMES DOS ARQUIVOS SALVOS AQUI
def write_results(result, reviews, VectorSize, WindowSize, Metric):
	prefix = f"v{VectorSize}_w{WindowSize}_m{Metric}"

	print ('Saving train.txt file...')
	f = open(f"train_{prefix}.txt", "w")
...
if __name__ == "__main__":
	# args = read_args()
	# m = int(args.method)
	#uncoment this to train word2vect of these mean vectors. Let's break down the key parts of the function:

	# FAZER O FOR AQUI

	for VectorSize in [100, 200, 300]:
		for WindowSize in [5, 10, 15]:
			for Metric in [1, 2]:
				prefix = f"v{VectorSize}_w{WindowSize}_m{Metric}"
				print(f"Running {prefix}")
				
				model = read_and_train(VectorSize,WindowSize)
				#save_model(model)
				#model = open_model()
				reviews = read_csv()
				results = extract_features_mean(model, reviews, Metric)
				write_results(results, reviews, VectorSize, WindowSize, Metric)
